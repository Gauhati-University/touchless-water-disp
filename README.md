...
1. The transmitter transmit a high frequency signal of 40kHz.
...
After the determination of the distance, the faucet of the Solenoid valve needs to  open or close, depending upon the control signal from the Arduino, is done using a relay by connecting or disconnecting the valve to the AC source. The AC source is connected to `C` terminal and the Solenoid valve to the `NO` terminal of the relay as shown in the figure. Here the relay, used in normally open configuration, upon recieving a HIGH signal at the `Signal` terminal from the Arduino allows current to flow from `C` terminal to `NO` terminal. 

<img src = "https://user-images.githubusercontent.com/63898803/83359725-bd54da80-a399-11ea-89f4-378f80060193.jpeg" width=400>

A transistor is used from between the Arduino and the relay as the transistor can source more current as compared to that of an Arduino and saturates the voltage across the coil faster. This leads to faster switching of the relay. Also it protects the Arduino from the reverse voltage across the relay coils generated due to stored energy in the coils.
